  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-4xl text-ink tracking-wide uppercase">
            Quản lý người dùng
          </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
           <div className="relative w-full sm:w-64 rounded-sm border border-admin-border bg-white overflow-hidden">
             <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-admin-subtext" size={16} />
             <input 
               type="text" 
           ...
           <button className="inline-flex items-center justify-center gap-2.5 rounded-sm bg-ink py-2.5 px-6 text-center font-bold text-white hover:bg-ink/80 transition-all text-xs uppercase tracking-widest">
      ...
      <div className="rounded-sm border border-admin-border bg-white shadow-sm mt-6">
        <div className="overflow-x-auto">
          <table className="w-full table-auto text-left">
            <thead>
              <tr className="bg-admin-primary-light border-b border-admin-border">
                ...
                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-admin-primary-light rounded-full flex items-center justify-center font-bold text-admin-subtext overflow-hidden border border-admin-border shrink-0 text-xs">
                  ...
                  <td className="py-4 px-6">
                    <span className={`text-[10px] font-bold uppercase tracking-widest py-1 px-2.5 rounded-full ${user.role === 'ADMIN' ? 'bg-ink text-volt' : 'bg-admin-primary-light text-admin-subtext'}`}>
              ...
              <div className="flex items-center gap-1">
                {[...Array(pagination.lastPage)].map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 text-[11px] font-bold rounded transition-all ${pagination.page === i + 1 ? 'bg-ink text-white' : 'hover:bg-gray-200 text-gray-600'}`}
                  >
                    {i + 1}
                  </button>
