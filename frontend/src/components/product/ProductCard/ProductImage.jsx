...
const ProductImage = ({
  ...
  return (
    <div className="relative overflow-hidden bg-gray-100 aspect-square">
      ...
      <div className={`absolute inset-0 bg-linear-to-t from-black/60 via-black/30 to-transparent flex items-center justify-center gap-4 transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'
        }`}>
        <button
        ...
        <button
          onClick={handleLike}
          className={`p-3 rounded-full transition-all duration-300 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white ${isLiked
              ? 'bg-red-500 hover:bg-red-600'
              : 'bg-white hover:bg-gray-100'
            }`}
          aria-label={isLiked ? "Bỏ thích" : "Thêm vào yêu thích"}
        >
          <Heart className={`w-5 h-5 ${isLiked ? 'text-white fill-white' : 'text-gray-900'
            }`} />
        </button>
      </div>
    </div>
